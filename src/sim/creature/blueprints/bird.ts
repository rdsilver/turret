/**
 * BIRD — a flapping flyer. It comes in high and fast over the field (about
 * twice walking pace), level and steady, swooping only slowly and gently. Its
 * wings carry it: shoot one off (or weaken its shoulder) and it can't hold
 * altitude any more; it glides down nose first and is stopped when it lands.
 * Lead it: aim at the near wing about half a body length ahead of the bird,
 * where the wing will be when the round gets there. A body or head shot also
 * works — if you can hit something that small.
 */
...
registerCreature('bird', (d, _rng, params) => {
  ...
  d.box(0, H, 0.9, 0.34, 'wood', { id: 'body', densityScale: 0.45, tags: ['core'], hpScale: P('bodyHp', 1.1) });
  d.circle(-0.58, H + 0.1, 0.17, 'wood', { id: 'head', densityScale: 0.4, hpScale: 1 });
  ...
  const wingHp = P('wingHp', 0.7);
  // Light wings (feathers, not planks): their beat barely shakes the body, and
  // the flicking hands stay light enough not to read as impacts (dust puffs).
  const wingD = P('wingD', 0.1);
  for (const side of ['L', 'R']) {
    const tags = side === 'R' ? ['limb', 'wing', 'back'] : ['limb', 'wing'];
    d.strut(shoulder[0], shoulder[1], elbow[0], elbow[1], 0.13, 'wood', { id: `wing${side}1`, densityScale: wingD, tags, hpScale: wingHp });
    d.strut(elbow[0], elbow[1], tip[0], tip[1], 0.09, 'wood', { id: `wing${side}2`, densityScale: wingD * 0.3, tags, hpScale: wingHp });
  }
  ...
  // Positive swings a wing back and down: a stroke from raised overhead to
  // down below the body. The wing sweeps down spread out, and comes back up
  // with the hand folded back (the elbow flexes most mid-upstroke).
